<template>
  <view class="w-172" v-if="item && item.cover">
    ...
    <image v-else :src="item.cover.itemType == 2 ? item.cover.content : item.cover.thumbnail" :style="{width:'100%',height:computedHeight(item.cover.width,item.cover.height)+'rpx'}"></image>
  </view>
