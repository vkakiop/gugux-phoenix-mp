			...
			<view v-for="(item,index) in data.column" :key="index" class="waterfalls-flow-column" :style="{'width':w,'margin-left':index==0?0:m}" :id="`waterfalls_flow_column_${index+1}`">
				<view class="column-value" v-for="(item2,index2) in data[`column_${index+1}`]" :key="index2">
					<view class="" v-if="item2.title">
						<view class="" v-if="item2.cover.itemType==2">
							<image :src="item2.cover.content" mode="widthFix" @load="imgLoad(item2)" @error="imgError(item2)" class="imgsty"></image>
						</view>
						<view class="" v-else>
							<image :src="item2.cover.thumbnail" mode="widthFix" @load="imgLoad(item2)" @error="imgError(item2)" class="imgsty"></image>
						</view>
